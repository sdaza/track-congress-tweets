...
I analyzed `r nrow(wdat)` tweets from Democrats and Republican members. Most of the tweets come from Democrats (`r round(nrow(wdat[party == "D"])/nrow(wdat), digits = 2)`), and the House of Representatives (`r round(nrow(wdat[chamber == "house"])/nrow(wdat), digits = 2)`). Below I show the distribution of the polarity scores.
...
The average of the polarity score is `r round(Mean(wdat[, polarity]), 2)`.  This means that on average each tweet is rather neutral, although they slightly incline towards more positive words. Democrats seem to have a higher proportion of *neutral* and negative words, what is confirmed by comparing the average polarity by party:  `r round(Mean(wdat[party == "D", polarity]), 2)` for Democrats, and `r round(Mean(wdat[party == "R", polarity]), 2)` for Republicans.
...
```{r, include = FALSE}
save(wdat, file = "tweet_data.Rdata")
```
